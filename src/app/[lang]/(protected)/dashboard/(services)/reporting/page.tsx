...
export default function ReportingDashboard() {
  ...
  const handleStepClick = (step: number) => {
    if (step === currentStep && stepStatus === 'pending') {
      switch (step) {
        case 1:
          generateReports()
          break
    ...
    >
      ...
      >
        ...
        >
          ...
          <PipelineStep name='Load' step={3} currentStep={currentStep} status={stepStatus} onClick={() => handleStepClick(3)} disabled={currentStep !== 3} />
        </motion.div>
      </motion.div>
      ...
      {showDetails &&
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="load">
            {currentStep === 3 && stepStatus === 'complete' && (
              <motion.div variants={slideUp} className="bg-white p-6 rounded-lg shadow-lg text-center">
                <CheckCircle2 className='w-24 h-24 text-green-500 mx-auto mb-4' />
                <p className='text-2xl font-semibold text-gray-700'>
                  Reports have been successfully loaded.
                </p>
              </motion.div>
            )}
          </TabsContent>
