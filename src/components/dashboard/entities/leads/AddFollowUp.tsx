...
import { follow_up_schema } from "@/lib/entities/schemas/lead"
...
import { CreateLeadFollowUp } from '@/utils/entities/lead'
...
const AddFollowUp = ({ leadId, onSuccess }: Props) => {
  const [isOpen, setIsOpen] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)

  const form = useForm({
    resolver: zodResolver(follow_up_schema),
    defaultValues: {
      date: new Date(),
      description: '',
      completed: false,
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      await CreateLeadFollowUp(leadId, followUpPayload)
