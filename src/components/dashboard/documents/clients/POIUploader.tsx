...
import { POIDocumentInfo } from '@/lib/entities/document'
import { File as DocumentFile } from '@/lib/entities/document'
import { UploadTicketPOIDocument } from '@/utils/entities/ticket'
import POIForm from './POIForm'
import { Account, IndividualAccountApplicationInfo } from '@/lib/entities/account'
import { UploadAccountPOIDocument } from '@/utils/entities/account'
...
const POIUploader = ({ onUploadSuccess, account, accountInfo }: Props) => {
    ...
    const handleSubmit = async (values: any) => {
        ...
        try {
            ...
            const documentInfo: POIDocumentInfo = {
                'type': values.type,
            ...
            const created_poi_id = await UploadAccountPOIDocument(customFile, documentInfo, session.user.id, account.id)
            if (!created_poi_id) throw new Error("Failed to upload document")
    ...
    return (
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="max-w-[50%]">
                <DialogHeader>
                    <DialogTitle>Upload a Proof of Identity</DialogTitle>
                    <DialogDescription>
                ...
                <POIForm onSubmit={(values) => handleSubmit(values)} uploading={uploading} />
            </DialogContent>
        </Dialog>
    )
}
